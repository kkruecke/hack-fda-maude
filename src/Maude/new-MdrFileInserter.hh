...
class MdrFileInserter extends AbstractFileInserter implements DatabaseUpdateInterface { 
  ...
  private $confirm_exists;
  private $newest_mdr_report_key; // that was just inserted when DeviceFileInserter.php was run. 
  private \ImmSet<int> $newest_mdr_report_keys; // that was just inserted when DeviceFileInserter.php was run. 
 ...
 public function setUp()
  {           
      $this->newest_mdr_report_key = Registr::registry('newest_mdr_report_key');
 
      $this->getPDO()->query("SELECT mdr_report_key from foi_device where mdr_report_key >= $this->newest_mdr_report_key ORDER BY mdr_report_key ASC");

      $newest_foidev_mdr_report_keys = $select_stmt->fetchAll(\PDO::FETCH_COLUMN, 0);
 
      $this->newest_mdr_report_keys->fromArrays($newest_foidev_mdr_report_keys); 

      /* bind insert statement parameters */     
  ...
  public function processLine($text, $line_number)
  {
      ...
      /* This test is simply an optimization to improve preformance. We skip rows where the mdr_report_key is less than the first 
         We skip rows where the mdr_report_key is not the same or greater than the first (and smallest) mdr_report_key that was just
         inserted by DeviceFileInserter.php.  */

      if ($this->mdr_report_key < $this->newest_report_keys) { 

           return;
      }               

     if (!$this->newest_mdr_report_keys.contains($this->mdr_report_key)) {
